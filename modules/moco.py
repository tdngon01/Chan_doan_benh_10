...
class GoogLeNetBackbone(nn.Module):
    def __init__(self, mlp, pretrained):
        ...
        self.backbone = googlenet(weights=weights, aux_logits=True)
        feature_dim = self.backbone.fc.in_features
...
class MoCo(nn.Module):
    def __init__(self,
        ...
        if self.backbone == "EfficientNet":
            ...
            self.encoder_q = GoogLeNetBackbone(pretrained=pretrained, mlp=mlp)
            self.encoder_k = GoogLeNetBackbone(pretrained=pretrained, mlp=mlp)
