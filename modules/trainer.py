...
def train(model, train_loader, criterion, optimizer, scaler, device, amp_enabled, epoch, total_epochs, train_mode_name, writer_tb):
    model.train()
    total_loss = 0.0
    ...
    for i, (images, labels) in enumerate(pbar):
        ...
        tag_prefix = "FULL" if train_mode_name == "Full Fine-Tune" else "LORA"

        writer_tb.add_scalar(
            f"Memory/{tag_prefix}_VRAM_MB",
            vram_mb,
            global_step
        )
...
def run_training(*,model, train_loader, val_loader, optimizer, device, log_dir, tensorboard_dir,
    ...
    last_ckpt, train_mode_name, resume_path=None):
    ...
    with open(metrics_csv_path, csv_mode, newline="", encoding="utf-8") as csv_file:
        ...
        for epoch in range(start_epoch, total_epochs):
            ...
            avg_loss = train(
                ...
                device, amp_enabled, epoch, total_epochs, train_mode_name, writer_tb
            )
            # Validation
            ...
            if mean_auc > best_auc:
                ...
                save_checkpoint(
                    save_path=os.path.join(checkpoint_dir, f"{epoch+1}_{best_ckpt}_{best_auc:.4f}.pth.tar"),
                    epoch=epoch + 1,
                    model=model,
                    optimizer=optimizer,
                    scheduler=scheduler,
            ...
            save_checkpoint(
                save_path=os.path.join(checkpoint_dir, f"{last_ckpt}.pth.tar"),
                epoch=epoch + 1,
                model=model,
                optimizer=optimizer,
                scheduler=scheduler,
