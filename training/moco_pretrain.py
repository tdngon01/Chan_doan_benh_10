...
def main():
    ...
    # Danh sách 6 backbone cần pre-train
    backbones = ["EfficientNet", "ResNet", "DenseNet", "MobileNet", "GoogleNet", "VGG16"]
    
    for backbone in backbones:
        print(f"\n======================================================================")
        print(f" BẮT ĐẦU TIỀN HUẤN LUYỆN BACKBONE: {backbone}")
        print(f"======================================================================")
        
        # Cập nhật cấu hình động cho backbone hiện tại
        ...
        # Tạo thư mục log cho backbone hiện tại
        LOG_DIR = os.path.join(cfg.LOGS_DIR, f"pretrain_{backbone}")
        ...
        # Khởi tạo mô hình MoCo với backbone hiện tại
        model_moco = MoCo().to(device)
        print(f"Model MoCo, kiến trúc {backbone} đã được khởi tạo")

        criterion = nn.CrossEntropyLoss().to(device) # loss
        optimizer = torch.optim.SGD( # hàm tối ưu
        ...
        if os.path.exists(resume_path):
            print(f"\nTìm thấy checkpoint cũ cho {backbone}: {resume_path}")
            ...
            print(f"\nKhông tìm thấy checkpoint cho {backbone}. Bắt đầu huấn luyện mới.")

        # Nếu mô hình hiện tại đã hoàn thành đủ số epochs, bỏ qua và chạy mô hình tiếp theo
        if start_epoch >= cfg.PRETRAIN_CONFIG["EPOCHS"]:
            print(f"Backbone {backbone} đã hoàn thành đủ {cfg.PRETRAIN_CONFIG['EPOCHS']} epochs. Chuyển sang mô hình tiếp theo.")
            continue

        print(f"\nBắt đầu huấn luyện {backbone} với MoCo...\n")
        ...
        with open(METRICS_CSV_PATH, csv_mode, newline="", encoding="utf-8") as f:
            ...
            for epoch in range(start_epoch, cfg.PRETRAIN_CONFIG["EPOCHS"]):
                ...
                pbar = tqdm(
                    moco_dataloader,
                    desc=f"[{backbone}] Epoch {epoch+1}/{cfg.PRETRAIN_CONFIG['EPOCHS']}",
                )
                ...
                print(f"[{backbone}] Epoch {epoch+1} hoàn thành. Loss: {avg_loss:.4f}. Thời gian: {duration:.1f}s")
                scheduler.step()
                ...
                if (epoch + 1) == cfg.PRETRAIN_CONFIG["EPOCHS"]:
                    ...
                    print(f"Đã lưu checkpoint định kỳ cuối cùng cho {backbone}: {save_path}")

        print(f"\nHuấn luyện xong Backbone: {backbone}\n")

    print("\nHuấn luyện tất cả 6 Backbone hoàn tất!")
